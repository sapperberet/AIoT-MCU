...
void loop() {
  ...
  if (!(client.connected())) {
    if ((now - lastReconnectAttempt) > 5000) {  /* Retry to connect every 5 seconds */
  ...
  if ((now - lastSensorPublish) > 2000) {  /* Sending data every 2 seconds */
    ...
    /* Sending sensors' data to the server */
